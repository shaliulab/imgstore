...
from .util import (
    ...
    ensure_color,
    ensure_grayscale,
    motif_extra_data_h5_to_df,
    motif_extra_data_json_to_df,
    motif_extra_data_h5_attrs,
)
...
class _ImgStore(object):
        ...
        **kwargs
        ...
        if mode == "w":
            if None in (imgshape, imgdtype, chunksize, format):
                raise ValueError(
                    "imgshape, imgdtype, chunksize, format must not be None"
                )
            self._frame_n = 0
            self._init_write(
                imgshape,
    ...
    def _readability_check(self, smd_class, smd_version):
        ...
        if smd_class != class_name:
            raise ValueError(
                "incompatible store, can_read:%r opened:%r"
                % (class_name, smd_class)
            )
    ...
    def _init_read(self):
        ...
        self._readability_check(
            smd_class=smd["class"], smd_version=smd["version"]
        )
        ...
        if "created_utc" not in smd:
            self._log.info(
                "old store detected. synthesizing created datetime / timezone"
            )
            ...
            # first the filename
            m = re.match(
                r"""(.*)(20[\d]{6}_\d{6}).*""", os.path.basename(self._basedir)
            )
            if m:
            ...
            self._log.debug(
                "parsed created_utc: %s (from %r)" % (_dt.isoformat(), _dt)
            )
            try:
                self._created_utc = _dt.astimezone(
                    pytz.utc
                )  # aware object can be in any timezone
            except ValueError:  # naive
                self._created_utc = _dt.replace(
                    tzinfo=pytz.utc
                )  # d must be in UTC
        ...
        for e in (encoding, write_encode_encoding):
            if e:
                if not self._codec_proc.check_code(e):
                    raise ValueError(
                        "unsupported store image encoding: %s" % e
                    )
    ...
    def _iter_chunk_n_and_chunk_paths(self):
        ...
        for cn, cp in sorted(
            self._chunk_n_and_chunk_paths, key=operator.itemgetter(0)
        ):
            yield cn, cp
    ...
    def _iter_extra_data_files(self, extensions=EXTRA_DATA_FILE_EXTENSIONS):
        for chunk_n, chunk_path in self._iter_chunk_n_and_chunk_paths():
            ...
            for ext in (".extra_data.h5", ".extra_data.json"):
                ...
                path = os.path.join(
                    self._basedir, "%06d%s" % (int(chunk_n), ext)
                )
                if os.path.exists(path):
                    yield True, path
                    break
    ...
    def find_extra_data_files(self, extensions=EXTRA_DATA_FILE_EXTENSIONS):
        return [
            path
            for _, path in self._iter_extra_data_files(extensions=extensions)
        ]
    ...
    def get_extra_data_samplerate(self):
        for c in self.find_extra_data_files(extensions=(".extra_data.h5",)):
            ...
            if len(ds) > 1:
                raise ValueError(
                    "Multiple sample rates detected: please use motif_extra_data_h5_attrs"
                )
            elif len(ds) == 1:
                try:
    ...
    def get_extra_data(self, ignore_corrupt_chunks=False):
        ...
        for is_motif, path in self._iter_extra_data_files():
            ...
            self._log.debug(
                "found extra data chunk: %s (motif: %s)" % (path, is_motif)
            )
            ...
            try:
                ...
                if ignore_corrupt_chunks:
                    self._log.warn(
                        "chunk extra data %s is corrupt" % path, exc_info=True
                    )
                    continue
                else:
                    raise
    ...
    def add_extra_data(self, **data):
        ...
        data["frame_index"] = (
            self._frame_n - 1
        )  # we post-increment in add_frame
    ...
    def extract_only_frame(cls, full_path, frame_index, _smd=None):
        ...
        return cls._extract_only_frame(
    ...
    def get_next_image(self):
        ...
        if _VERBOSE_DEBUG_GETS:
            self._log.debug(
                "get_next_image frame_number: %s idx %s" % (frame_number, idx)
            )

        return self._get_image(
            chunk_n=None,  # chunk was already loaded in _get_next_framenumber_and_chunk_frame_idx
            frame_idx=idx,
        )
    ...
    def _get_image_by_frame_index(self, frame_index):
        ...
        self._log.debug(
            "seek found in chunk %d attempt read chunk index %d"
            % (self._chunk_n, frame_idx)
        )
    ...
    def _get_image_by_time(self, frame_time, exact_only=False):
        ...
        if chunk_n == -1:
            raise ValueError(
                "timestamp #%s not found in any chunk" % frame_time
            )
    ...
    def _get_image_by_frame_number(self, frame_number, exact_only):
        if _VERBOSE_DEBUG_CHUNKS:
            self._log.debug(
                "seek by frame_number %s (exact: %s)"
                % (frame_number, exact_only)
            )
    ...
    def get_image(self, frame_number, exact_only=True, frame_index=None):
        ...
        if _VERBOSE_DEBUG_GETS:
            self._log.debug(
                "get_image %s (exact: %s) frame_idx %s"
                % (frame_number, exact_only, frame_index)
            )
        if frame_index is not None:
            return self._get_image_by_frame_index(frame_index)
        else:
            return self._get_image_by_frame_number(
                frame_number, exact_only=exact_only
            )
    ...
    def close(self, save_index=False):
        if self._mode in "wa":
            ...
            try:
                if os.path.isfile(
                    os.path.join(self._basedir, STORE_LOCK_FILENAME)
                ):
                ...
                # noinspection PyArgumentList
                self._log.warn(
                    "could not remove lock file (unknown error)", exc_info=True
                )

            if save_index:
                index = ImgStoreIndex.new_from_chunks(
                    self._find_chunks(chunk_numbers=None)
                )
                index.to_file(
                    os.path.join(self._basedir, STORE_INDEX_FILENAME)
                )
    ...
    def reindex(self):
        ...
        zero_idx = fn.index(0)
        self._log.info(
            "reindexing about frame_number=0 at index=%d" % zero_idx
        )
        ...
        for chunk_n, chunk_path in self._iter_chunk_n_and_chunk_paths():
            ...
            nfn = fn_new[
                chunk_n * self._chunksize : (chunk_n * self._chunksize)
                + self._chunksize
            ]
                ...
                if os.path.exists(ed_path):
                    ...
                    try:
                        ...
                        if "frame_index" not in df.columns:
                            raise ValueError(
                                "can not reindex extra-data on old format stores"
                            )
                        df["_frame_number_before_reindex"] = df["frame_number"]
                        df["frame_number"] = df.apply(
                            lambda r: fn_new[int(r.frame_index)], axis=1
                        )
                        with open(ed_path, "w") as f:
                            df.to_json(f, orient="records")
                        self._log.debug(
                            "reindexed chunk %d metadata (%s)"
                            % (chunk_n, ed_path)
                        )
                    except Exception:
                        self._log.error(
                            "could not update chunk extra data to new framenumber",
                            exc_info=True,
                        )
...
def _extract_ranges(data):
    ...
    for key, group in itertools.groupby(
        enumerate(sorted(data)), lambda x: x[0] - x[1]
    ):
...
class DirectoryImgStore(_ImgStore):
    ...
    def __init__(self, framerate=None, isColor=False, **kwargs):
        ...
        if (self._mode == "w") and (self._format == "pgm") and self._color:
            self._log.warn(
                "store created with color image shape but using grayscale 'pgm' format"
            )
        ...
        if (
            (self._format == "npy")
            and (np.__version__ < "1.9.0")
            and (self._mode in "wa")
        ):
    ...
    def _save_image(self, img, frame_number, frame_time):
        dest = os.path.join(
            self._chunk_cdir,
            "%06d.%s" % (self._frame_n % self._chunksize, self._format),
        )
    ...
    def _save_chunk(self, old, new):
        ...
        if new is not None:
            ...
            self._new_chunk_metadata(
                os.path.join(self._basedir, "%06d" % new, "index")
            )
    ...
    def _find_chunks(self, chunk_numbers):
        if chunk_numbers is None:
            ...
            chunk_numbers = list(
                map(int, immediate_dirs)
            )  # N.B. need list, as we iterate it twice
        return list(
            zip(
                chunk_numbers,
                tuple(
                    os.path.join(self._basedir, "%06d" % int(n), "index")
    ...
    def _open_image(path, format, color):
        if format in DirectoryImgStore._cv2_fmts:
            ...
            with open(path, "rb") as reader:
                img = bloscpack.numpy_io.unpack_ndarray(
                    bloscpack.file_io.CompressedFPSource(reader)
                )
        else:
    ...
    def _load_image(self, idx):
        ...
        return img, (
            self._chunk_md["frame_number"][idx],
            self._chunk_md["frame_time"][idx],
        )
    ...
    def _load_chunk(self, n):
        ...
        self._chunk_n = n
        self._chunk_current_frame_idx = (
            -1
        )  # not used in DirectoryImgStore, but maintain compat
...
class VideoImgStore(_ImgStore, ImgStoreExport):
    ...
    def __init__(self, framerate=25, isColor=False, **kwargs):
        ...
        if self._supports_seeking:
            ...
            self._log.info(
                "seeking NOT enabled on store (will fallback to sequential reading)"
            )

        if self._mode == "r":
            if self.supports_format(self._format) or (
                (self._metadata.get("class") == "VideoImgStoreFFMPEG")
                and (("264" in self._format) or ("nvenc-" in self._format))
            ):
                check_imgshape = self._calculate_written_image_shape(
                    self._imgshape, ""
                )
            else:
            ...
            if check_imgshape != self._imgshape:
                self._log.warn(
                    "previous store had incorrect image_shape: corrected %r -> %r"
                    % (self._imgshape, check_imgshape)
                )
                self._imgshape = check_imgshape
            self._color = (self._imgshape[-1] == 3) & (
    ...
    def _find_chunks(self, chunk_numbers):
        if chunk_numbers is None:
            avis = map(
                os.path.basename,
                glob.glob(os.path.join(self._basedir, "*%s" % self._ext)),
            )
            chunk_numbers = list(
                map(
                    int,
        ...
        return list(
            zip(
                chunk_numbers,
                tuple(
                    os.path.join(self._basedir, "%06d" % n)
                    for n in chunk_numbers
                ),
            )
        )
    ...
    def _extract_only_frame(basedir, chunk_n, frame_n, smd):
        capfn = os.path.join(
            basedir,
            "%06d%s" % (chunk_n, VideoImgStore._get_chunk_extension(smd)),
        )
        # noinspection PyArgumentList
        ...
        if _VERBOSE_DEBUG_CHUNKS:
            ...
            log.debug(
                "opening %s chunk %d frame_idx %d" % (capfn, chunk_n, frame_n)
            )
    ...
    def insert_chunk(self, video_path, frame_numbers, frame_times, move=True):
        ...
        self._new_chunk_metadata(
            os.path.join(self._basedir, "%06d" % self._chunk_n)
        )
        ...
        vid = os.path.join(
            self._basedir, "%06d%s" % (self._chunk_n, self._ext)
        )
        if move:
...
def _parse_basedir_fullpath(path, basedir, read):
    ...
    if basedir:
        if basedir.endswith(STORE_MD_FILENAME) and (
            os.path.basename(basedir) == STORE_MD_FILENAME
        ):
            raise ValueError(
                "basedir should be a directory (not a file ending with %s)"
                % STORE_MD_FILENAME
            )
    ...
    if (
        path
        and path.endswith(STORE_MD_FILENAME)
        and (os.path.basename(path) == STORE_MD_FILENAME)
    ):
        ...
        if read:
            if os.path.isdir(path) and os.path.exists(
                os.path.join(path, STORE_MD_FILENAME)
            ):
                basedir = path
        ...
        raise ValueError(
            "should be a path to a store %s file or a directory containing one"
            % STORE_MD_FILENAME
        )
...
def new_for_filename(path, **kwargs):
    ...
    basedir, fullpath = _parse_basedir_fullpath(
        path, kwargs.pop("basedir", None), read=kwargs.get("mode") == "r"
    )
    ...
    try:
        cls = {
            DirectoryImgStore.__name__: DirectoryImgStore,
            VideoImgStore.__name__: VideoImgStore,
        }[clsname]
    except KeyError:
...
def new_for_format(fmt, path=None, **kwargs):
    ...
    # we are writing mode
    basedir, _ = _parse_basedir_fullpath(
        path, kwargs.pop("basedir", None), read=kwargs.get("mode") == "r"
    )
    kwargs["basedir"] = basedir
...
def extract_only_frame(path, frame_index):
    ...
    return cls.extract_only_frame(
        full_path=fullpath, frame_index=frame_index, _smd=smd
    )
