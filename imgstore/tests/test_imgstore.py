...
def encode_image(num, nbits=16, imgsize=512):
    ...
    row = np.fromiter(
        (255 * int(c) for c in ("{0:0%db}" % nbits).format(num)),
        dtype=np.uint8,
    )
    mat = np.tile(row, (nbits, 1))
    return cv2.resize(
        mat, dsize=(imgsize, imgsize), interpolation=cv2.INTER_NEAREST
    )
...
def decode_image(img, nbits=16, imgsize=512):
    ...
    img = cv2.resize(
        img, dsize=(nbits, nbits), interpolation=cv2.INTER_NEAREST
    )
...
def new_framecode_video(dest, frame0, nframes):
    ...
    cap = cv2.VideoWriter(
        dest, FourCC("M", "J", "P", "G"), 25, (512, 512), isColor=True
    )
...
def grey_image():
    cimg = cv2.imread(
        os.path.join(TEST_DATA_DIR, "graffiti.png"), cv2.IMREAD_GRAYSCALE
    )
...
def graffiti():
    return cv2.imread(
        os.path.join(TEST_DATA_DIR, "graffiti.png"), cv2.IMREAD_COLOR
    )
...
def test_all(tmpdir, fmt):
    ...
    d = stores.new_for_filename(
        os.path.join(d.filename, stores.STORE_MD_FILENAME), mode="r"
    )
...
def test_outoforder(tmpdir, fmt, imgtype):
    ...
    assert not os.path.isfile(
        os.path.join(d.filename, stores.STORE_LOCK_FILENAME)
    )

    d = stores.new_for_filename(
        os.path.join(d.filename, stores.STORE_MD_FILENAME)
    )
    ...
    npt.assert_array_equal(
        sorted([frame_times[f] for f in fns]), sorted(md["frame_time"])
    )
    ...
    cmp_ratio = abs(
        100 - (100.0 * ((len(fns) * float(orig_img.nbytes)) / sz_bytes))
    )
...
def test_create_and_times(loglevel_debug, tmpdir):
    ...
    d = stores.new_for_filename(
        os.path.join(TEST_DATA_DIR, "store_mp4", "metadata.yaml")
    )
    c_utc, tz = d.created
...
def test_extract_only(loglevel_debug, fmt, tmpdir):
    ...
    d, _ = new_framecode_store(dest=tdir, frame0=57, nframes=20, format=fmt)
...
def test_manual_assembly(loglevel_debug, tmpdir):
    ...
    a = new_framecode_video(
        dest=os.path.join(tdir, "a.avi"), frame0=0, nframes=10
    )

    a_fns = list(range(0, 10))  # [0 ... 9]

    b = new_framecode_video(
        dest=os.path.join(tdir, "b.avi"), frame0=57, nframes=20
    )

    b_fns = list(range(57, 57 + 20))  # [57 ... 76]

    dest = os.path.join(tdir, "store")
    store = stores.VideoImgStore(
        basedir=dest,
        mode="w",
        imgshape=(512, 512, 3),
        imgdtype=np.uint8,
    ...
    for f in fns:
        img, (frame_number, frame_time) = store.get_image(
            frame_number=f, exact_only=True
        )
...
def test_store_frame_metadata(tmpdir):
    ...
    for f in range(N):
        img, (frame_number, frame_time) = d.get_image(
            frame_number=f, exact_only=False
        )
...
def test_videoimgstore_mp4(seek):
    ...
    d = stores.new_for_filename(
        os.path.join(TEST_DATA_DIR, "store_mp4", "metadata.yaml"), seek=seek
    )
...
def test_reindex_to_zero(loglevel_debug, tmpdir, grey_image, chunksize):
    ...
    npt.assert_array_equal(
        s.get_frame_metadata()["frame_number"], [3, 4, 0, 1, 2, 3, 4]
    )
    ...
    npt.assert_array_equal(
        j.get_frame_metadata()["frame_number"], [-2, -1, 0, 1, 2, 3, 4]
    )
...
def test_seek_types(loglevel_debug, tmpdir, chunksize, fmt, seek):
    ...
    for i in range(2000, 2100):
        if i in skips:
            continue
        d.add_image(
            ensure_color(encode_image(i, imgsize=512)),
            frame_number=i,
            frame_time=time.time(),
        )
    d.close()
    ...
    # check seeking works
    frame, (frame_number, frame_timestamp) = d.get_image(
        frame_number=2000, exact_only=True
    )
    assert frame_number == 2000
    assert _decode_image(frame) == 2000

    frame, (frame_number, frame_timestamp) = d.get_image(
        frame_number=2001, exact_only=True
    )
    ...
    with pytest.raises(ValueError):
        frame, (frame_number, frame_timestamp) = d.get_image(
            frame_number=skips[0], exact_only=True
        )

    # check seeking works by frame_index
    for i in range(10, 15):
        frame, (frame_number, frame_timestamp) = d.get_image(
            frame_number=None, exact_only=True, frame_index=i
        )
    ...
    frame, (frame_number, frame_timestamp) = d.get_image(
        frame_number=None, exact_only=True, frame_index=29
    )
    assert frame_number == 2029
    assert _decode_image(frame) == 2029

    frame, (frame_number, frame_timestamp) = d.get_image(
        frame_number=None, exact_only=True, frame_index=30
    )
    assert frame_number == 2070
    assert _decode_image(frame) == 2070

    frame, (frame_number, frame_timestamp) = d.get_image(
        frame_number=None, exact_only=True, frame_index=31
    )
    assert frame_number == 2071
    assert _decode_image(frame) == 2071

    frame, (frame_number, frame_timestamp) = d.get_image(
        frame_number=None, exact_only=True, frame_index=0
    )
    assert frame_number == 2000
    assert _decode_image(frame) == 2000
    ...
    # last frame
    frame, (frame_number, frame_timestamp) = d.get_image(
        frame_number=None, exact_only=True, frame_index=frame_count - 1
    )
    ...
    with pytest.raises(ValueError):
        frame, (frame_number, frame_timestamp) = d.get_image(
            frame_number=None, exact_only=True, frame_index=frame_count
        )
...
def test_new_apis(tmpdir):
    ...
    fullp = os.path.join(p, stores.STORE_MD_FILENAME)
    d = stores.new_for_format(
        fmt="npy", path=p, imgshape=(10, 10), imgdtype=np.uint8
    )
    assert d.full_path == fullp
    assert d.mode == "w"
    d.close()
    ...
    fullp = os.path.join(p, stores.STORE_MD_FILENAME)
    d = stores.new_for_format(
        fmt="npy", path=fullp, imgshape=(10, 10), imgdtype=np.uint8
    )
    assert d.full_path == fullp
    assert d.mode == "w"
    d.close()
    ...
    fullp = os.path.join(p, stores.STORE_MD_FILENAME)
    d = stores.new_for_format(
        fmt="npy", basedir=p, imgshape=(10, 10), imgdtype=np.uint8
    )
    assert d.full_path == fullp
    assert d.mode == "w"
    d.close()
    ...
    with pytest.raises(ValueError):
        ...
        fullp = os.path.join(p, stores.STORE_MD_FILENAME)
        d = stores.new_for_format(
            fmt="npy", basedir=fullp, imgshape=(10, 10), imgdtype=np.uint8
        )
...
def test_odd_sized(fmt, tmpdir):
    ...
    d = stores.new_for_format(
        fmt, basedir=tmpdir.strpath, imgshape=img.shape, imgdtype=img.dtype
    )
    for i in range(10):
        d.add_image(img, i, 0.0)
    d.close()
...
def test_complex_framenumber(tmpdir, chunksize, fmt):
    s_full_path, times = _non_monotonic_store_with_wrap(
        fmt, tmpdir.strpath, chunksize, save_index=False
    )
    ...
    # old behaviour
    frame, (frame_number, frame_timestamp) = d.get_image(
        42, exact_only=False, frame_index=None
    )
    ...
    for fn in reversed(NMTW_FNS):
        frame, (frame_number, frame_timestamp) = d.get_image(
            fn, exact_only=True, frame_index=None
        )
        assert frame_number == fn
        assert decode_image(frame) == fn
    ...
    for fn in reversed(NMTW_FNS):
        ...
        frame, (frame_number, frame_timestamp) = d.get_image(
            frame_number=None, exact_only=True, frame_index=idx
        )
        assert frame_number == fn
        assert decode_image(frame) == fn
    ...
    # get nearest frames
    frame, (frame_number, frame_timestamp) = d.get_image(
        6054, exact_only=False, frame_index=None
    )
    ...
    frame, (frame_number, frame_timestamp) = d.get_nearest_image(
        times[0] - 1.0
    )
    assert frame_number == 6050
    assert decode_image(frame) == 6050
    frame, (frame_number, frame_timestamp) = d.get_nearest_image(
        times[0] + (3.0 * (times[1] - times[0]) / 4.0)
    )
    assert frame_number == 6055
    assert decode_image(frame) == 6055
...
def test_index(tmpdir, chunksize):
    ...
    basedir = tmpdir.strpath
    s_full_path, times = _non_monotonic_store_with_wrap(
        "npy", basedir, chunksize, save_index=True
    )

    assert os.path.exists(os.path.join(basedir, STORE_INDEX_FILENAME))

    idx = ImgStoreIndex.new_from_file(
        os.path.join(basedir, STORE_INDEX_FILENAME)
    )
...
def test_max_framenumber_behaviour(fmt, nframes, seek, tmpdir):
    _, path = new_framecode_store(
        dest=tmpdir.strpath, frame0=3, nframes=nframes, format=fmt, chunksize=3
    )

    store = stores.new_for_filename(path, seek=seek)
    assert store.frame_count == nframes

    img, (_fn, _) = store.get_image(
        frame_number=store.frame_max, exact_only=True
    )
    ...
    img, (_fn, _) = store.get_image(
        frame_number=store.frame_max - 1, exact_only=True
    )
    ...
    with pytest.raises(ValueError):
        store.get_image(
            frame_number=None, exact_only=True, frame_index=store.frame_count
        )
